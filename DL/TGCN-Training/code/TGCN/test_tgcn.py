...
if __name__ == '__main__':
    ...
    checkpoint = 'ckpt.pth'
    # path changes according to the trained_on and tested_on subsets

    split_file = os.path.join(root, 'data/splits/asl100.json')
    ...
    model = GCN_muti_att(input_feature=num_samples * 2, hidden_feature=hidden_size,
                         num_class=int(trained_on[3:]), p_dropout=drop_p, num_stage=num_stages).to(device)

    print('Loading model...')

    checkpoint = torch.load(os.path.join(root, 'code/TGCN/archived/asl100/ckpt.pth'), map_location=device)
